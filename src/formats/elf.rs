...
pub fn parse_elf_header(elf_data: &[u8]) -> Result<ELFHeader, StructureError> {
    ...
    if elf_header.padding.iter().all(|&x| x == 0)
        ...
        && let Some(elf_class) = parse_elf_class(elf_header.class)
        && let Some(osabi) = parse_elf_osabi(elf_header.osabi)
    {
...
const fn parse_elf_class(elf_class: u8) -> Option<u8> {
    match elf_class {
        1 => Some(32),
        2 => Some(64),
        _ => None,
...
const fn parse_elf_type(elf_type: u16) -> Option<&'static str> {
    Some(match elf_type {
        0 => "no file type",
        1 => "relocatable",
        2 => "executable",
